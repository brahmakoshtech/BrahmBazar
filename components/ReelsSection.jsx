...
function ReelCard({ reel }) {
    ...
    const handleReelClick = () => {
        router.push(`/reels?id=${reel._id}`);
    };

    return (
        <div
            onClick={handleReelClick}
            className="flex-shrink-0 w-[200px] h-[320px] md:w-[220px] md:h-[360px] relative rounded-xl overflow-hidden group transition-all duration-500 hover:scale-105 hover:shadow-[0_20px_50px_rgba(234,88,12,0.3)] cursor-pointer"
        ...
        >
            ...
            <div className="relative w-full h-full rounded-[0.7rem] overflow-hidden bg-black">
                ...
                <div className={`absolute inset-0 flex items-center justify-center bg-black/20 transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
                    <div className="px-4 py-2 bg-white/20 backdrop-blur-md rounded-full border border-white/30 text-white font-medium text-sm flex items-center gap-2">
                ...
                {reel.title && (
                    <div className="absolute bottom-0 left-0 right-0 p-4 z-20 bg-gradient-to-t from-black/80 to-transparent">
                        <p className="text-white text-sm font-serif font-bold line-clamp-2 leading-tight">
                            {reel.title}
                        </p>
                    </div>
                )}
